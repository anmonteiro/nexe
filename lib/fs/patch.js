...
var nexeBinary = null;
function restoreFs(fs) {
    if (fs === void 0) { fs = require('fs'); }
    if (!nexeBinary) {
        return false;
    }
    var source = nexeBinary;
...
function shimFs(binary, fs) {
    ...
    nexeBinary = binary;
    var blobPath = binary.blobPath, manifest = binary.resources;
    var _a = binary.layout, resourceStart = _a.resourceStart, stat = _a.stat;
    var directories = {}, isString = function (x) { return typeof x === 'string' || x instanceof String; }, isNotFile = function () { return false; }, isNotDirectory = isNotFile, isFile = function () { return true; }, noop = function () { }, isDirectory = isFile, path = require('path');
